...
const (
	...
	// --userspec` (which, unlike util-linux's setpriv/runuser, shares no package
	// with the setuid binary), so grep's (sticky) AccessedByRoot stays false.
	nonRootPackage = "grep"
)
...
func (s *packageInUseSuite) TestPackageInUse() {
	...
	s.Run("in-use", func() {
		...
		s.EventuallyWithTf(func(collect *assert.CollectT) {
			...
			assert.Equalf(c, "true", s.packageProperty(ubiRepo, inUsePackage, propRunningAsRoot), "gzip RunningAsRoot should be true (ubi9 workload runs as root)")
			assert.Equalf(c, "false", s.packageProperty(ubiRepo, inUsePackage, propHasSetSuidBit), "gzip HasSetSuidBit should be false (gzip is not setuid)")
	...
	s.Run("stale-after-stop", func() {
		...
		s.EventuallyWithTf(func(collect *assert.CollectT) {
			...
			ts, _, _ := s.packageUsage(c, ubiRepo, inUsePackage)
	...
	// Phase 4: security properties. Cover the property values the gzip phases do
	// not: HasSetSuidBit == "true" (su is setuid root, owned by util-linux) and
	// RunningAsRoot == "false" (grep is run only as the unprivileged nobody user).
	s.Run("security-properties", func() {
		s.startSecurityProbes()
		defer s.stopSecurityProbes()

		s.EventuallyWithTf(func(collect *assert.CollectT) {
			c := &myCollectT{CollectT: collect, errors: []error{}}
			collect = nil //nolint:ineffassign

			suidTS, suidPresent, _ := s.packageUsage(c, ubiRepo, suidPackage)
			require.Truef(c, suidPresent, "%s carries no %s property yet", suidPackage, propLastSeenRunning)
			require.Positivef(c, suidTS, "%s was never reported in use", suidPackage)

			nrTS, nrPresent, _ := s.packageUsage(c, ubiRepo, nonRootPackage)
			require.Truef(c, nrPresent, "%s carries no %s property yet", nonRootPackage, propLastSeenRunning)
			require.Positivef(c, nrTS, "%s was never reported in use", nonRootPackage)

			suidVal := s.packageProperty(ubiRepo, suidPackage, propHasSetSuidBit)
...
// named package's component (newest access wins), whether that property was
// present on any payload, and the names of all components currently reported as
// in use (a diagnostic for when the targeted package is not the one that flipped).
func (s *packageInUseSuite) packageUsage(c *myCollectT, ubiRepo, pkg string) (maxTS int64, present bool, inUse []string) {
...
// package's component from the most recently collected ubi9 SBOM payload, or ""
// if absent.
func (s *packageInUseSuite) packageProperty(ubiRepo, pkg, name string) string {
	ids, err := s.Fakeintake.GetSBOMIDs()
	if err != nil {
		return ""
	}
...
func (s *packageInUseSuite) startSecurityProbes() {
	...
	stdout, stderr := s.podExec("sh", "-c", script)
	s.T().Logf("PKG-IN-USE start security probes: stdout=%q stderr=%q", stdout, stderr)
}
